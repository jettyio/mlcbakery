...
async def verify_admin_token(credentials: HTTPAuthorizationCredentials = Depends(bearer_scheme)):
    """
    ...
    is_token_valid = secrets.compare_digest(
        credentials.credentials, admin_auth_token # Compare against the locally read token
    )
