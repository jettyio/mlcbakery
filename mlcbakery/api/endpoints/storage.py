...
@router.post(
    "/datasets/{collection_name}/{dataset_name}/data", 
...
async def upload_dataset_data(
    ...
    if collection.storage_provider != "gcp":
        raise HTTPException(
            status_code=400, 
            detail="Only 'gcp' storage provider is currently supported"
        )
    ...
    try:
        ...
        uploaded_path = upload_file_to_gcs(bucket_name, file_content, destination_path, gcs_client)
...
async def download_latest_dataset_data(
    ...
    try:
        ...
        if latest_file_number < 0:
            raise HTTPException(status_code=404, detail="No data files found for this dataset")
