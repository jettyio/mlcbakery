...
from ...models import TrainedModel, Collection
...
async def create_trained_model(
    ...
    if trained_model.collection_id:
        ...
        if not result_coll.scalar_one_or_none():
            raise HTTPException(status_code=404, detail=f"Collection {trained_model.collection_id} not found")

    db_trained_model = TrainedModel(**trained_model.model_dump())
    db.add(db_trained_model)
    await db.commit()
    await db.refresh(db_trained_model)
    stmt_refresh = select(TrainedModel).where(TrainedModel.id == db_trained_model.id).options(selectinload(TrainedModel.collection))
    result_refresh = await db.execute(stmt_refresh)
    db_trained_model = result_refresh.scalar_one()
...
async def list_trained_models(
    ...
    stmt = select(TrainedModel).where(TrainedModel.entity_type == 'trained_model').offset(skip).limit(limit).options(selectinload(TrainedModel.collection))
    result = await db.execute(stmt)
    trained_models = result.scalars().all()
    return trained_models


@router.get("/trained-models/{trained_model_id}", response_model=TrainedModelResponse)
async def get_trained_model(trained_model_id: int, db: AsyncSession = Depends(get_async_db)):
    ...
    stmt = select(TrainedModel).where(TrainedModel.id == trained_model_id).where(TrainedModel.entity_type == 'trained_model').options(selectinload(TrainedModel.collection))
    result = await db.execute(stmt)
...
@router.delete("/trained-models/{trained_model_id}", status_code=200)
