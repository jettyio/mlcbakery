...
async def create_trained_model(
    ...
    if trained_model.collection_id:
        stmt_coll = select(Collection).where(
            Collection.id == trained_model.collection_id
        )
        result_coll = await db.execute(stmt_coll)
        if not result_coll.scalar_one_or_none():
            raise HTTPException(
                status_code=404,
                detail=f"Collection with id {trained_model.collection_id} not found",
            )
    ...
    stmt_refresh = (
        ...
        .options(
            ...
            selectinload(
                TrainedModel.input_activities
            ).options(  # Load activities where this dataset is an input
            ...
            selectinload(
                TrainedModel.output_activities
            ).options(  # Load the activity that *created* this dataset
                selectinload(Activity.input_entities).options(
                    selectinload(Entity.collection)
                ),
                selectinload(Activity.agents),
    ...
    if not refreshed_trained_model:
        raise HTTPException(
            status_code=500, detail="Failed to reload trained model after creation"
        )
...
async def get_trained_model(
    trained_model_id: int, db: AsyncSession = Depends(get_async_db)
):
