    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        if coll_get_resp.status_code == 200 and coll_get_resp.json():
            for coll in coll_get_resp.json():
                ...
                datasets_in_coll_resp = await ac.get(
                    f"/api/v1/datasets/?collection_id={coll['id']}"
                )
                if datasets_in_coll_resp.status_code == 200:
                    for ds_in_coll in datasets_in_coll_resp.json():
                        ...
                        if ds_in_coll["name"] in dataset_names:
                            print(
                                f"Defensive cleanup: Deleting dataset {ds_in_coll['id']} ('{ds_in_coll['name']}')"
                            )
                            await ac.delete(
                                f"/api/v1/datasets/{ds_in_coll['id']}",
                                headers=AUTH_HEADERS,
                            )

                print(
                    f"Defensive cleanup: Deleting collection {coll['id']} ('{coll['name']}')"
                )
                del_coll_resp = await ac.delete(
                    f"/api/v1/collections/{coll['id']}", headers=AUTH_HEADERS
                )
                # Ignore 404 if already gone between check and delete
                assert del_coll_resp.status_code in [200, 404], (
                    f"Cleanup failed deleting collection {coll['id']}"
                )
        ...
        collection_data = {
            ...
            "description": "For upstream tree ds test",
        }
        coll_resp = await ac.post(
            "/api/v1/collections/", json=collection_data, headers=AUTH_HEADERS
        )
        assert coll_resp.status_code == 200, (
            f"Failed creating collection after cleanup: {coll_resp.text}"
        )
        collection_id_to_use = coll_resp.json()["id"]
        ...
        ds1_resp = await ac.post(
            "/api/v1/datasets/", json=ds1_data, headers=AUTH_HEADERS
        )
        ds2_resp = await ac.post(
            "/api/v1/datasets/", json=ds2_data, headers=AUTH_HEADERS
        )
        ...
        ds3_resp = await ac.post(
            "/api/v1/datasets/", json=ds3_data, headers=AUTH_HEADERS
        )
        assert ds3_resp.status_code == 200
        ds3_id = ds3_resp.json()["id"]
        act1_data = {
            "name": "Activity 1",
            "input_entity_ids": [ds1_id, ds2_id],
            "output_entity_id": ds3_id,
        }
        act1_resp = await ac.post(
            "/api/v1/activities/", json=act1_data, headers=AUTH_HEADERS
        )
        ...
        ds4_resp = await ac.post(
            "/api/v1/datasets/", json=ds4_data, headers=AUTH_HEADERS
        )
        assert ds4_resp.status_code == 200
        ds4_id = ds4_resp.json()["id"]
        act2_data = {
            "name": "Activity 2",
            "input_entity_ids": [ds3_id],
            "output_entity_id": ds4_id,
        }
        act2_resp = await ac.post(
            "/api/v1/activities/", json=act2_data, headers=AUTH_HEADERS
        )
        ...
        print(
            f"Requesting upstream tree URL: {upstream_url}"
        )  # Add print for debugging
        # Make the GET request using the name-based URL
        response = await ac.get(upstream_url)
        # Update assertion message
        assert response.status_code == 200, (
            f"Request to {upstream_url} failed: {response.status_code} - {response.text}"
        )
        ...
        assert (
            len(tree_data.get("children", [])) == 1
        )  # Should have DS3 (via Activity 2) as input
