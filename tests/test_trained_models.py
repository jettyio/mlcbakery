...
async def test_create_trained_model():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        response = await ac.post("/api/v1/trained_models/", json=trained_model_data, headers=AUTH_HEADERS)
...
async def test_get_trained_model():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        create_resp = await ac.post("/api/v1/trained_models/", json=model_data, headers=AUTH_HEADERS)
        assert create_resp.status_code == 200
        model_id = create_resp.json()["id"]

        # Get the model
        response = await ac.get(f"/api/v1/trained_models/{model_id}", headers=AUTH_HEADERS)
...
async def test_list_trained_models():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        for i in range(3):
            ...
            resp = await ac.post("/api/v1/trained_models/", json=model_data, headers=AUTH_HEADERS)
...
async def test_delete_trained_model():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        # Delete the model
        response = await ac.delete(f"/api/v1/trained_models/{model_id}", headers=AUTH_HEADERS)
        assert response.status_code == 200, f"Delete failed: {response.text}"
        assert response.json()["message"] == "Trained model deleted successfully"

        # Verify the model is deleted
        get_response = await ac.get(f"/api/v1/trained_models/{model_id}", headers=AUTH_HEADERS)
